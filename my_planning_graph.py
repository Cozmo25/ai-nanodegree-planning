...
class PgNode_a(PgNode):
    ...
    def precond_s_nodes(self):
        ...
        """
        nodes = set()
        for precond in self.action.precond_pos:
            nodes.add(PgNode_s(precond, True))
        for precond in self.action.precond_neg:
            nodes.add(PgNode_s(precond, False))
        return nodes
    ...
    def effect_s_nodes(self):
        ...
        """
        nodes = set()
        for effect in self.action.effect_add:
            nodes.add(PgNode_s(effect, True))
        for effect in self.action.effect_rem:
            nodes.add(PgNode_s(effect, False))
        return nodes
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if node_a.prenodes.issubset(self.s_levels[level]):
                ...
                # make connections to previous S nodes
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Make sure node_a1's (+) effect aren't canceled by node_a2's (-) preconds
        for effect in node_a1.action.effect_add:
        ...
        # Make sure node_a2's (+) effect aren't canceled by node_a1's (-) preconds
        for effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Make sure node_a1's actions aren't canceled by node_a2's actions
        for action_a1 in node_a1.parents:
            for action_a2 in node_a2.parents:
                if action_a1.is_mutex(action_a2):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for state_s1 in node_s1.parents:
            for state_s2 in node_s2.parents:
                if not state_s1.is_mutex(state_s2):
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        #    Bug in this implementation as it returns 13 steps instead of 12 - see below for correct implementation
        #    goal_found = False

        #    for level in range(len(self.s_levels)):
        #        for state in self.s_levels[level]:
                    #print("Goal: " + str(goal))
                    #print("Symbol: " + str(state.symbol))
                    #print("Is pos: " + str(state.is_pos))
        ...
        def cost_helper(goal):
            for idx, lvl in enumerate(self.s_levels):
                if PgNode_s(goal, True) in lvl:
                    return idx

        for goal in self.problem.goal:
            level_sum += cost_helper(goal)
